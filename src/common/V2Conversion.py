...
for e_id, e in okr.entities.iteritems():
    ...
    mentions = {m_id: NodeMention(m_id, m.sentence_id, m.indices, m.terms, new_entity_id) for m_id, m in
                e.mentions.iteritems()}
...
for p_id, p in okr.propositions.iteritems():
    new_p_id = "P" + str(p_id)
    prop_mentions = {
    m_num: [[num, pos.orth_, pos.tag_] for num, pos in enumerate(nlp(unicode(" ".join(okr.sentences[m.sentence_id]))))
            if num in m.indices] for m_num, m in p.mentions.iteritems() if m.is_explicit}
    new_terms = {m_num: " ".join([str(word[1]) for word in m if word[2] not in S_WORDS]) for m_num, m in
                 prop_mentions.iteritems()}
    ...
    new_mentions = {m_num: NodeMention(m_num, m.sentence_id, new_indices[m_num], new_terms[m_num], new_p_id) for
                    m_num, m in p.mentions.iteritems() if m.is_explicit}
    if (not (len(new_terms_all) == 1 and [n for n in new_terms_all][0] == "")) and len(
            new_terms_all) > 0:  # not empty predicate
    ...
    new_terms_edge = {m_num: " ".join([str(word[1]) for word in m if word[2] in S_WORDS]) for m_num, m in
                      prop_mentions.iteritems()}

    # replace predicates with nodes:
    new_templates_w_args = {m_num: change_template_predicate(m.template, new_terms.get(m_num, None), new_p_id) for
                            m_num, m in p.mentions.iteritems()}
    # replace arguments:
    new_templates = {m_num: change_template_arguments(template, p.mentions[m_num].argument_mentions) for m_num, template
                     in new_templates_w_args.iteritems()}
    ...
    main_predicates = {m_num: new_p_id if not new_terms.get(m_num, "") == "" else None for m_num, m in
                       p.mentions.iteritems()}
    # extract edge mentions:
    Edge_Mentions = Edge_Mentions + [
        EdgeMention(new_p_id + "_" + str(m_num), m.sentence_id, new_indices_edge.get(m_num, -1),
                    new_terms_edge.get(m_num, ""), new_templates[m_num],
                    extract_nodes_from_templates(new_templates[m_num]), main_predicates[m_num], embedded[m_num],
                    m.is_explicit) for m_num, m in p.mentions.iteritems()]
Nodes = {}
...
for edge_mention in Edge_Mentions:
    if edge_mention.main_pred == None:
        ...
        pairs = [(edge_mention.main_pred, node) for node in edge_mention.all_nodes if
                 not edge_mention.main_pred == node]
    for pair in pairs:
        edge_id = "_".join(pair)
